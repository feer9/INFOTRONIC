...
#define __write_pin__(port,pin,state)	((state) ?	(FIOSET[8*port] = (1UL << pin)) : \
									 				(FIOCLR[8*port] = (1UL << pin)))
#define write_pin(...)			__write_pin__(__VA_ARGS__)

#define __set_pin__(port,pin)	( FIOSET[8*(port)] = (1UL << (pin)) )
#define set_pin(...)			__set_pin__(__VA_ARGS__)

#define __clear_pin__(port,pin)	( FIOCLR[8*(port)] = (1UL << (pin)) )
#define clear_pin(...)			__clear_pin(__VA_ARGS__)

#define __read__(port,pin)		( ( FIOPIN[8*(port)] >> (pin) ) & 1UL )
#define read(...)				__read__(__VA_ARGS__)

#define __read_pin__(port,pin,activ)	((((FIOPIN[8*(port)] >> (pin)) & 1UL) == (activ)) ? 1:0 )
#define read_pin(...)			__read_pin__(__VA_ARGS__)

#define __set_dir__(port,pin,dir)	( (dir)	?	( FIODIR[8*(port)] |=  (1UL << (pin)) ) : \
												( FIODIR[8*(port)] &= ~(1UL << (pin)) ) )
#define set_dir(...)			__set_dir__(__VA_ARGS__)

#define __toggle_pin__(port,pin)	( FIOPIN[8*(port)] ^= (1UL << (pin)) )
