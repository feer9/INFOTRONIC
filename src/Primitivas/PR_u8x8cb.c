...
uint8_t u8x8_gpio_and_delay_lpc1769(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr)
{
	...
	case U8X8_MSG_GPIO_AND_DELAY_INIT:
		...
		setPINSEL(KEY_SELECT, PINSEL_GPIO);
		setPINMODE(KEY_SELECT, PINMODE_PULLUP);
		set_dir_input(KEY_SELECT);
/*
		setPINSEL(KEY_PREV, PINSEL_GPIO);
		setPINMODE(KEY_PREV, PINMODE_PULLUP);
		set_dir_input(KEY_PREV);

		setPINSEL(KEY_NEXT, PINSEL_GPIO);
		setPINMODE(KEY_NEXT, PINMODE_PULLUP);
		set_dir_input(KEY_NEXT);

		setPINSEL(KEY_HOME, PINSEL_GPIO);
		setPINMODE(KEY_HOME, PINMODE_PULLUP);
